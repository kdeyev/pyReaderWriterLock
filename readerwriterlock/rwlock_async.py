...
class RWLockRead(RWLockable):
	...
	class _aReader(Lockable):
		...
		async def release(self) -> None:
			...
			self.v_locked = False
			await asyncio.shield(self.c_rw_lock.c_lock_read_count.acquire())
			self.c_rw_lock.v_read_count -= 1
	...
	class _aWriter(Lockable):
		...
		async def acquire(self, blocking: bool = True, timeout: float = -1) -> bool:
			...
			try:
				...
				await asyncio.shield(self.c_rw_lock.c_lock_write_count.acquire())
				self.c_rw_lock.v_write_count -= 1
...
class RWLockReadD(RWLockableD):
	...
	class _aReader(Lockable):
		...
		async def acquire(self, blocking: bool = True, timeout: float = -1) -> bool:
			...
			try:
				...
				return False
			await asyncio.shield(self.c_rw_lock.v_read_count.increment())
			if 1 == int(self.c_rw_lock.v_read_count):
				try:
					await asyncio.wait_for(self.c_rw_lock.c_resource.acquire(), timeout=(None if c_deadline is None else max(sys.float_info.min, c_deadline - self.c_rw_lock.c_time_source())))
				except (asyncio.TimeoutError, asyncio.CancelledError):
					await asyncio.shield(self.c_rw_lock.v_read_count.decrement())
					self.c_rw_lock.c_lock_read_count.release()
					return False
			self.c_rw_lock.c_lock_read_count.release()
			self.v_locked = True
	...
	class _aWriter(LockableD):
		...
		async def downgrade(self) -> Lockable:
			...
			result = await asyncio.shield(self.c_rw_lock.gen_rlock())

			wait_blocking = asyncio.Event()

			async def lock_result() -> None:
				wait_blocking.set()
				await asyncio.shield(result.acquire())  # This is a blocking action
				wait_blocking.set()

			run_task(lock_result())

			await asyncio.shield(wait_blocking.wait())  # Wait for the thread to be almost in its blocking state.
			wait_blocking.clear()

			for _ in range(123):
				await asyncio.shield(asyncio.sleep(sys.float_info.min))  # Heuristic sleep delay to leave some extra time for the thread to block.

			await asyncio.shield(self.release())  # Open the gate! the current RW lock strategy gives priority to reader, therefore the result will acquire lock before any other writer lock.

			await asyncio.shield(wait_blocking.wait())  # Wait for the lock to be acquired
			return result
